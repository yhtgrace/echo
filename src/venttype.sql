...
select
  ...
  , max(
    case
      when itemid = 467 and value = 'Bipap Mask' THEN 1
      when itemid = 467 and value = 'CPAP Mask' THEN 1
      when itemid = 720 and value = 'CPAP' THEN 1
      when itemid = 720 and value = 'CPAP+PS' THEN 1
      else 0
      end
    ) as NonInvVent
